...
adb_bin=${ADB:-adb}
apk="$project_root/app/build/outputs/apk/playstore/debug/app-playstore-debug.apk"
test_apk="$project_root/app/build/outputs/apk/androidTest/playstore/debug/app-playstore-debug-androidTest.apk"
...
timeout 45s "$adb_bin" -s "$serial" shell am instrument -w -r \
  -e class Ali.kytyps4.FexGuestHarnessDeviceTest \
  Ali.kytyps4.test/androidx.test.runner.AndroidJUnitRunner >"$run_directory/instrumentation.raw" 2>&1
